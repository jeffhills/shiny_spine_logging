...
jh_make_op_note_test_df_function <- function(lumbar_pso_example = FALSE, posterior_or_anterior = "posterior", spine_region = "lumbar", object = c("pre_selected")){
  
  if(nrow(all_implants_constructed_df) < 600){
    jh_load_all_possible_objects_function()
  }
  if(lumbar_pso_example == TRUE){
    selected_objects_df <- all_implants_constructed_df %>%
      filter(object == "pedicle_screw", vertebral_number > 17) %>%
      filter(level != "L4") %>%
      union_all(all_implants_constructed_df %>%
                  filter(object == "pelvic_screw_1") %>%
                  filter(row_number() != 2)) %>%
      union_all(all_implants_constructed_df %>%
                  filter(object == "grade_3", level == "L4"))
  }else{
  ...
  selected_objects_df <- selected_objects_df %>%
    ...
    select(level, vertebral_number, body_interspace, approach, category, implant, object, side, fusion, screw_size_type, implant_statement)
...
jh_get_vertebral_number_function <- function(level_to_get_number){
  vert_number <-
    case_when(
      ...
      # level_to_get_number == 'L4-S1' ~ 23.75,
      level_to_get_number == 'L5-S1' ~ 24.5,
      # level_to_get_number == 'L5-L6' ~ 24.25,
...
fusion_levels_df_function <- function(all_objects_to_add_df){
  if(any(all_objects_to_add_df$fusion == "yes")){
    ...
    #   
    #   if(length(fusion_range_df$vertebral_number == 1)){
...
#                                                 revision_implants_retained_df = tibble(level = character(), vertebral_number = double(), x = double(), y = double(), prior_rod_connected = character()),
...
#       if(nrow(revision_rod_retained_df) > 0){
#         if(tail(revision_rod_retained_df$level, 1) == linked_rod_vector[2]){
#           revision_rod_is <-  "proximal"
#         }else if(head(revision_rod_retained_df$level, 1) == linked_rod_vector[1]){
...
#         revision_rod_is <-  "na"
#       }
#       main_rod_df <-  revision_rod_retained_df %>%
#         bind_rows(main_rod_df) %>%
#         arrange(vertebral_number) %>%
#         distinct()
#       
#       if(revision_rod_is == "proximal"){
#         proximal_rod_levels_vector <- (revision_rod_retained_df %>%
...
#         distal_rod_levels_vector <- (revision_rod_retained_df %>%
#                                          filter(prior_rod_connected == "yes"))$level
#         
#       }else{
...
#       #   # filter(side == rod_side, level %in% proximal_rod_levels_vector) %>%
#       #   filter(side == rod_side) %>%
#       #   filter(vertebral_number <= jh_get_vertebral_number_function(level_to_get_number = distal_rod_vector[[2]])) %>%
#       #   filter(vertebral_number >= min(main_rod_df$vertebral_number)) %>%
...
#       #   filter(side == rod_side, level %in% distal_rod_levels_vector) %>%
...
#       # distal_linked_rod_matrix <- all_screw_coordinates_df %>%
#       #   filter(side == rod_side) %>%
#       #   filter(vertebral_number >= jh_get_vertebral_number_function(level_to_get_number = distal_rod_vector[[1]])) %>%
#       #   filter(vertebral_number <= max(main_rod_df$vertebral_number)) %>%
#       #   mutate(x = if_else(level %in% proximal_rod_levels_vector, x + x_linked_rod_modifier, x)) %>%
#       #   select(x, y) %>%
...
#         intercalary_revision_rod_is <- "proximal_intercalary"
#       }else if(all(main_rod_df$vertebral_number < jh_get_vertebral_number_function(intercalary_rod_junction))){
#         intercalary_revision_rod_is <- "distal_intercalary"
#       }else{
#         intercalary_revision_rod_is <- "na"
#       }
# 
...
#       
#       if(intercalary_revision_rod_is != "proximal_intercalary"){
#         rods_list$intercalary_proximal_rod_sf <- jh_sf_rod_object_from_matrix_function(proximal_intercalary_rod_matrix)
#       }
#       if(intercalary_revision_rod_is != "distal_intercalary"){
#         rods_list$intercalary_distal_rod_sf <- jh_sf_rod_object_from_matrix_function(distal_intercalary_rod_matrix)
#       }
#       
#       rods_list$main_rod_sf <- NULL
...
jh_generate_supplemental_rod_statement_function <- function(rod_type = "accessory_rod",
  ...
  if(!is.null(rod_type) && rod_side %in% c("left", "right") && (length(rod_vector) == 2 && rod_vector[1] %in% all_screw_coordinates_df$level)){
    ...
    if(str_detect(rod_type, "linked")){
      rod_statement <- str_squish(glue("On the {rod_side} side, a linked rod configuration was used with {rod_size} {rod_material} rods overlapping from the {rod_vector[1]} to the {rod_vector[2]} levels."))
    }
