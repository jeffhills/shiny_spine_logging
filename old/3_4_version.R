...
server <- function(input, output, session) {
  ...
  output$left_accessory_rod_range_ui <- renderUI({
    if(req(input$left_accessory_rod_switch) == FALSE){
      ...
      sliderTextInput(
        ...
        choices = unique(implants_list$left_implants_df$level),
        selected = c(implants_list$left_implants_df$level[1], tail(implants_list$left_implants_df$level, n = 1)) #c(unique(implants_list$left_implants_df$level)[1], tail(unique(implants_list$left_implants_df$level), n = 1))
      )
    }
  })
  ...
  observeEvent(input$plot_click, {
    ...
    implant_type_filtered_df <- implant_added_reactive_df() %>%
      filter(implant == implant_type_selected()) %>%
      remove_empty() 
  ...
  ######################### NOW CREATE THE IMPLANTS #########################
  
  ## THIS WAS GOOD:
  left_implants_ready_for_construction_df <- reactive({
    left_implants_active_df <- implants_list$left_implants_df
    
    if(nrow(left_implants_active_df) >0){
      if(length(input$left_satellite_rod)>0){
        ...
        left_final_implants_df <- left_implants_active_df %>%
  ...
  left_implants_constructed_reactive_sf <- reactive({
    left_implants_active_df <- implants_list$left_implants_df
    
    if(nrow(left_implants_active_df) >0){
      
      left_implants_constructed_df <- left_implants_ready_for_construction_df() %>%
        mutate(implant_constructed = pmap(.l =  list(..1 = implant,
                                                     ...
                                                     ..5 = length,
                                                     ..6 = width,
                                                     ..7 = rod), .f = ~ screw_hook_implant_function(implant_type = ..1, main_or_sattelite_rod = ..7,
                                                                                                    start_x = ..2,
                                                                                                    y = ..3,
                                                                                                    angle = ..4,
                                                                                                    screw_length_mod = ..5,
      ...
      st_multipolygon(left_implants_constructed_df$implant_constructed)
  ...
  left_rods$left_main_rod_reactive_sf <- reactive({
    if(nrow(implants_list$left_implants_df) > 1){
      ...
      rod_matrix <- left_implants_ready_for_construction_df() %>%
        filter(rod == "main_rod") %>%
        select(x, y) %>%
        arrange(y) %>%
        distinct() %>%
  ...
  left_rods$left_accessory_rod_reactive_sf <- reactive({
    if(nrow(implants_list$left_implants_df) > 1){
      if(length(input$left_accessory_rod) > 1){
        
        rod_matrix <- tibble(levels = input$left_accessory_rod, side = "left") %>%
          left_join(left_implants_ready_for_construction_df()) %>%
          select(x, y) %>%
          mutate(x = x - 0.02) %>%
          arrange(y) %>%
          distinct() %>%
  ...
  observe({
    ...
    if(nrow(implants_list$right_implants_df) >0){
      if(length(input$right_satellite_rod)>0){
  ...
  right_rods$right_accessory_rod_reactive_sf <- reactive({
    if(nrow(implants_list$right_implants_df) > 1){
      if(length(input$right_accessory_rod) > 1){
        
        rod_matrix <- tibble(levels = input$right_accessory_rod, side = "right") %>%
          left_join(implants_list$right_implants_constructed_df) %>%
          select(x, y) %>%
          mutate(x = x + 0.02) %>%
          arrange(y) %>%
          distinct() %>%
  ...
  # 
  # # observeEvent(input$plot_double_click, {
  # #     req(nrow(implants_list$right_implants_constructed_df$implant_constructed) > 0)
  # #     
  # #     left_implants_ready_for_construction_df() <- left_implants_ready_for_construction_df() %>%
  ...
  # #         filter(side == "left")
  # #     
  # #     implants_list$right_implants_constructed_df <- implants_list$right_implants_constructed_df %>%
  # #         mutate(point_touches = map_lgl(.x = implant_constructed, .f = ~ st_contains(x = .x, y = point_to_test(), sparse = FALSE))) %>%
  # #         filter(point_touches == FALSE) %>%
  # #         filter(side == "right")
  # # })
  ...
  output$spine_plan <- renderPlot({
    ...
    left_implants_sf <- st_multipolygon(left_implants_constructed_df$implant_constructed)
    ...
    spine_plot <- ggdraw() +
      ...
      ggpattern::geom_sf_pattern(
        data = left_implants_constructed_reactive_sf(),
        pattern = "stripe",
        pattern_fill = "blue",
        alpha = 0.8,
        pattern_colour = "blue",
      ...
      theme_minimal_grid() +
      geom_sf(data = left_rods$left_main_rod_reactive_sf(), alpha = 0.5) +
  ...
  output$coordinates_table <- renderTable({
    ...
    implants_list$left_implants_df %>%
      select(level, vertebral_number, implant, side, x, y, rod)
