...
# source("dashboard_user_interface.R", local = TRUE)
                      ...
                      tabItems(
                                                    ...
                                                    pickerInput(
                                                      ...
                                                      choices = list('Axial Pain' = c("Neck Pain", "Thoracic Pain", "Low Back Pain"), 
                                                                     ...
                                                                     'Myelopathy' = c("Nurick Grade 1",
                                                                                      ...
                                                                                      "Nurick Grade 4",
                                                                                      "Nurick Grade 5"),
                                                                     'Deformity' = c("Coronal Deformity", "Sagittal Imbalance", "Chin on chest deformity"),
                                ...
                                column(width = 8, 
                                       box(width = 12, title = tags$div(style = "font-size:22px; font-weight:bold", "Surgical Procedures:"), solidHeader = TRUE, status = "primary",
                                                 ...
                                                 column(width =  11, 
                                                        tags$table(
                                                          tags$tr(width = "100%",
                                                                  ...
                                                                  tags$td(width = "70%", radioGroupButtons(inputId = "spine_approach", 
                                                                                                           label = NULL,
                                                                                                           choices = c("Anterior", "Posterior"),
                                                                                                           selected = "Posterior", 
                                                                                                           direction = "horizontal",justified = TRUE,
                                                                                                           checkIcon = list(yes = icon("fas fa-check-square", style = "color: steelblue")))))
                                                        ) 
                                                 ),
                                                 column(width = 1, 
                                                        dropdownButton(
                                           ...
                                           box(width = 5, ##### RIGHT COLUMN NEXT TO SPINE PLOT
                                               div(style = "font-size:16px; font-weight:bold; text-align:left", "Approach & Technique Specifics:"),
                        ...
                        tabItem(tabName = "operative_note",
                                ...
                                box(width = 5, title = div(style = "font-size:22px; font-weight:bold; text-align:center", "Additional Surgical Details:"),status = "info", solidHeader = TRUE,
                                    ...
                                    br(),
                                    div(class = "form-group shiny-input-container shiny-input-checkboxgroup shiny-input-container-inline", style = "width: 950%;text-align: -webkit-center;",
...
server <- function(input, output, session) {
  ...
  spine_plan_plot <- reactive({
    ...
    if(input$spine_approach == "Anterior"){
      ...
      ## SCREWS
      if(any(str_detect((all_objects_to_add_list$objects_df$object), pattern = "screw"))){
        screws_sf <- st_multipolygon((all_objects_to_add_list$objects_df %>% filter(str_detect(string = object, pattern = "screw")))$object_constructed)
      }else{
        screws_sf <- NULL
      }
      ...
      if(any(str_detect(all_objects_to_add_list$objects_df$object, pattern = "costo"))){
        costovertebral_sf <- st_multipolygon((all_objects_to_add_list$objects_df %>% filter(object == "costovertebral_approach" | object == "costotransversectomy"))$object_constructed)
      }else{
      ...
      if(any(str_detect(all_objects_to_add_list$objects_df$object, pattern = "intervertebral_cage"))){
        intervertebral_cage_sf <- st_union(st_combine(st_multipolygon((all_objects_to_add_list$objects_df %>% filter(object == "intervertebral_cage"))$object_constructed)), by_feature = TRUE, is_coverage = TRUE)
  ...
  screw_details_selected_reactive <- reactive({
    if(nrow(pedicle_screw_level_reactive_df() > 1)){
      max_levels <- nrow(pedicle_screw_level_reactive_df())
      levels_count <- seq(from = 1, to = max_levels, by = 1)
      
      screw_levels_labels_df <- pedicle_screw_level_reactive_df() %>%
      ...
      screw_details_df <- tibble(level = character(), side = character(), screw_diameter = character(), screw_type = character(), screw_size = character(), screw_size_type = character())
    }
  ...
  output$pedicle_screw_details_table <- renderTable({
    if(nrow(screw_details_selected_reactive())>0){
      screw_details_selected_reactive()
    }else{
  ...
  op_note_text_reactive <- reactive({
    ...
    if(nrow(posterior_approach_objects_df) > 0){
      ...
      if(any(str_detect(all_objects_to_add_list$objects_df$object, "screw"))){
        posterior_approach_objects_df <- all_objects_to_add_list$objects_df %>%
          filter(approach == "posterior") %>%
          left_join(screw_details_selected_reactive() %>% select(level, side, screw_size_type)) %>%
          filter(object != "intervertebral_cage") %>%
    ...
    secion_headers_df <- tibble(section = c("\nPatient:",
                                ...
                                result = c(paste(input$patient_first_name, input$patient_last_name),
                                           ...
                                           glue("{input$indications} resistant to conservative treatment"),
