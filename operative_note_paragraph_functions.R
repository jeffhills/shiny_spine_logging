...
procedure_classifier_type_df <- tribble(~object_name, ~procedure_label, ~paragraph_type, 
                                        ...
                                        'si_fusion_screw_1', 'SI Fusion', 'combine',
                                        'si_fusion_screw_2', 'SI Fusion', 'combine',
                                        'si_fusion_screw_3', 'SI Fusion', 'combine',
                                        'si_fusion_screw_4', 'SI Fusion', 'combine',
                                        ...
                                        'nerve_transection', 'Transection of spinal nerve', 'combine'
                                        )
...
jh_implant_technique_method_statement <- function(implant_technique_method = NULL) {
  ...
  techniques <- unique(tolower(implant_technique_method))
  
  has_prior_screw_tracts      <- "prior_screw_tracts"              %in% techniques
  has_free_hand       <- "free_hand"              %in% techniques
  has_fluoro_markers  <- "fluoro_pedicle_markers"%in% techniques
  has_fluoro_perc     <- "fluoro_perc"           %in% techniques
  ...
  has_nav_robot <- any(c("nav_robot_intra_ct", "nav_robot_pre_ct") %in% techniques)
  has_nav_3d    <- "nav_3d_surface_map" %in% techniques
  
  modality_phrases <- character(0)
  modality_descriptive_sentences <- character(0)
  
  if (has_prior_screw_tracts) {
    modality_phrases <- c(modality_phrases,
                          "prior screw tracts")
    
    modality_descriptive_sentences <- c(modality_descriptive_sentences,
                                        "Prior screw tracts were palpated for breaches and tapped as needed.")
  ...
  if (has_fluoro_perc) {
    ...
    modality_descriptive_sentences <- c(modality_descriptive_sentences, 
                                        paste("For percutaneous screw placement, the TP was palpated and a Jamshidi was placed at the junction of the TP and superior facet under X-ray guidance.",
                                              "The Jamshidi needle was then advanced down the pedicle and a wire placed through the pedicle and into the vertebral body and the pedicles were then sequentially tapped.",
                                              "X-ray was used frequently to confirm accurate screw placement."
                                        ))
  }
  ...
  if (has_nav_basic) {
    ...
    modality_descriptive_sentences <- c(modality_descriptive_sentences,
                                        "A reference array was secured to the patient and an intraoperative scan was obtained.")
    ...
    if("navigation_portion_screws" %in% techniques){
      modality_descriptive_sentences <- c(modality_descriptive_sentences,
                                          "Navigated instruments were used for placing select screws.")
    }
    if("navigation" %in% techniques){
      modality_descriptive_sentences <- c(modality_descriptive_sentences,
                                          "Navigated instruments were used for placing the screws.")
    }
  }
  ...
  if (has_nav_robot) {
    ...
    if("nav_robot_intra_ct" %in% techniques){
      modality_descriptive_sentences <- c(modality_descriptive_sentences,
                                          "A reference array was secured to the patient and an intraoperative scan was obtained. Screws were then placed with robotic assistance.")
    }
    if("nav_robot_pre_ct" %in% techniques){
      modality_descriptive_sentences <- c(modality_descriptive_sentences,
                                          "A reference array was secured to the patient and intraoperative X-ray's were used to merge with the preoperative CT scan. Screws were then placed with robotic assistance.")
    }
  }
  if (has_nav_3d) {
    modality_phrases <- c(modality_phrases,
                          "intraoperative navigation via 3D surface mapping")
    
    modality_descriptive_sentences <- c(modality_descriptive_sentences,
                                        "A reference array was secured to the patient and optical surface mapping images were merged with a CT scan for navigation assistance. Screws were then placed with navigated instruments. ")
  }
  ...
  if (length(modality_phrases) == 1) {
    techniques_sentence <- glue::glue(
      "Screws were placed using {modality_phrases}."
    )
  } else {
    techniques_sentence <- glue::glue(
      "Screws were placed using a combination of {glue::glue_collapse(modality_phrases, sep = ', ', last = ' and ')}."
    )
  }
  
  detailed_modality_sentences <- glue_collapse(modality_descriptive_sentences, sep = " ")
  
  full_technique_text <- str_squish(paste(techniques_sentence, detailed_modality_sentences))
...
op_note_procedure_paragraphs_function <- function(objects_added_df, 
  ...
  # print(paste("ALL IMPLANT METHODS SHOULD PRINT HERE111: ", glue_collapse(implant_technique_method)))
  ...
  paragraph_df_prepped_df <- df_for_paragraphs %>%
    mutate(approach_technique_input = approach_technique,
           ...
           # method_implant_start_point = screw_technique_statement, 
  ...
  procedures_op_full_df <-  paragraph_df_prepped_df %>%
    ...
    mutate(procedure_statement = pmap(list(..1 = procedure_category,
                                           ...
                                           ..5 = neuromonitoring_emg_statement_input,
                                           # ..6 = method_implant_start_point,
                                           ..6 = method_implant_confirmation),
                                      .f = ~create_full_paragraph_statement_function(procedure_paragraph_intro = ..1,
                                                                                     df_with_levels_object_nested = ..2,
                                                                                     paragraphs_combined_or_distinct = ..3,
                                                                                     approach_technique = ..4,
                                                                                     neuromonitoring_emg = ..5,
                                                                                     # implant_technique_method = ..6,
                                                                                     implant_confirmation_method = ..6))) %>%
    select(procedure_category, procedures_combine, procedure_statement) %>%
    unnest(procedure_statement)
  
  if(any(str_detect(procedures_op_full_df$procedure_statement, "SI_SCREW_TECHNIQUE_PLACEHOLDER"))){
    if(any(str_detect(implant_technique_method, "nav_pelvic|navigation"))){
      si_screw_tech <-  "Navigation guidance was used to identify the start points for each SI fusion implant. Trajectories were confirmed in multiple planes, and implants were advanced across the SI joint under navigation guidance. "
    }else{
     si_screw_tech <-  "SI fusion implants were placed using standard technique with intraoperative imaging to confirm start points and trajectories. Implants were advanced across the SI joint under X-ray guidance and final implant position was confirmed. "
    }
    
    procedures_op_full_df <- procedures_op_full_df %>%
      mutate(procedure_statement = str_replace_all(procedure_statement, "SI_SCREW_TECHNIQUE_PLACEHOLDER", si_screw_tech))
    
  }
  
  if(any(str_detect(procedures_op_full_df$procedure_statement, "PELVIC_SCREW_NAVIGATION_PLACEHOLDER"))){
    if("nav_pelvic_screws" %in% implant_technique_method){
      pelvic_screws_nav_technique <- "Pelvic screws were placed using navigation assistance. A reference frame was attached to the patient and navigated instruments were used to confirm the start point and trajectory. "
    }else{
      pelvic_screws_nav_technique <- ""
    }
  ...
  # print(dput(procedures_op_full_df))
...
create_full_paragraph_statement_function <- function(procedure_paragraph_intro, 
  ...
  if(paragraphs_combined_or_distinct == "combine"){
    ...
    if(length(implant_technique_method) == 0){
      implant_technique_method <- ""
    }
    
    # print(paste("ALL IMPLANT METHODS SHOULD PRINT HERE: ", glue_collapse(implant_technique_method)))
    ...
    levels_txt <- glue::glue_collapse(unique(df_levels$level), sep = ", ", last = ", and ")
    
    ## EMG USED:
    emg_statement <- if_else(neuromonitoring_emg != "No", neuromonitoring_emg, "")
    

    implant_confirmation_method_text <- if_else(
      procedure_paragraph_intro %in% c("posterior spinal instrumentation", "pelvic instrumentation", "si fusion") &
        str_length(implant_confirmation_method) > 6,
      implant_confirmation_method,
      ""
    )
    
    ## 6) Completion sentence via case_when (handles special cases first, then default)
    procedure_completion <- case_when(
      procedure_paragraph_intro == "posterior spinal instrumentation" ~
        glue("{implant_confirmation_method_text} {emg_statement} This partially completed the {procedure_paragraph_intro} of {levels_txt}."),
      procedure_paragraph_intro == "incision and drainage" ~
        glue("This completed the {procedure_paragraph_intro}."),
      ...
      procedure_paragraph_intro == "posterior column osteotomy" & length(unique(df_levels$level)) > 1 ~
        glue("This completed the posterior column osteotomies at {levels_txt}."),
      procedure_paragraph_intro == "reinsertion of spinal fixation" ~
        glue("This completed the reinsertion of spinal fixation at {levels_txt}."),
      procedure_paragraph_intro == "pelvic instrumentation" ~
        glue("{implant_confirmation_method_text} This completed the instrumentation of the pelvis."),
      procedure_paragraph_intro == "si fusion" ~
        glue("{implant_confirmation_method_text} This completed the fusion of the SI joint with transfixation device."),
      procedure_paragraph_intro == "transection of spinal nerve" ~ 
        glue(" "),
      TRUE ~
    ...
    statement <- case_when(
      ...
      procedure_paragraph_intro == "reinsertion of spinal fixation" ~
        glue("I reinserted screws at {levels_txt}. {glue_collapse(df_with_statement$tech_statement, sep = ' ')} {procedure_completion}"),
      procedure_paragraph_intro == "si fusion" ~
        glue("I then proceeded with the SI fusion with fixation device. {glue_collapse(df_with_statement$tech_statement, sep = ' ')} {procedure_completion}"),
      procedure_paragraph_intro == "transection of spinal nerve" ~
        glue("{glue_collapse(df_with_statement$tech_statement, sep = ' ')}"),
      TRUE ~
        glue("I then proceeded with the {procedure_paragraph_intro} at {levels_txt}. {glue_collapse(df_with_statement$tech_statement, sep = ' ')} {procedure_completion}")
    )
...
if(str_detect(object, "si_fusion")){
  
  si_implant_technique_statement <- "SI_SCREW_TECHNIQUE_PLACEHOLDER"
  
 si_screws_wide <- levels_side_df %>%
    select(level, side) %>%
    mutate(count = row_number()) %>%
    pivot_wider(names_from = side, values_from = count) %>%
    unnest(everything())

 si_screws_statement_list <- list()
 # si_screws_statement_list$technique <- glue("")
 
 left_si_screws_df <- levels_side_df %>%
   filter(level == "SI", side == "left")
 
 
 if(nrow(left_si_screws_df)>0){
   left_si_screw_sizes <- glue_collapse(left_si_screws_df$screw_size_type, sep = " and ")
   
   if(length(left_si_screws_df$screw_size_type)>1){
     si_screws_statement_list$left_si_screws <- str_to_sentence(glue("On the left, {left_si_screw_sizes} implants were placed across the SI joint."))
   }else{
     si_screws_statement_list$left_si_screws <- str_to_sentence(glue("An {left_si_screw_sizes} SI joint fixation implant was placed across the SI joint."))
   }
 }
 
 right_si_screws_df <- levels_side_df %>%
   filter(level == "SI", side == "right")
 
 if(nrow(right_si_screws_df)>0){
   right_si_screw_sizes <- glue_collapse(right_si_screws_df$screw_size_type, sep = " and ")
   
   if(length(right_si_screws_df$screw_size_type)>1){
     si_screws_statement_list$right_si_screws <- str_to_sentence(glue("On the right, {right_si_screw_sizes} implants were placed across the SI joint."))
   }else{
     si_screws_statement_list$right_si_screws <- str_to_sentence(glue("An {right_si_screw_sizes} SI joint fixation implant was placed across the SI joint."))
   }
 }
 
 si_fusion_screw_statement <- paste(si_implant_technique_statement, glue_collapse(si_screws_statement_list, sep = " "))
} else{
  si_fusion_screw_statement <- glue(" ")
  ...
  if (str_detect(object, "pelvic_screw")) {
    ...
    #   pelvic_screws_list$nav_technique <- "PELVIC_SCREW_NAVIGATION_PLACEHOLDER "
    # }
    pelvic_screws_list$nav_technique <- "PELVIC_SCREW_NAVIGATION_PLACEHOLDER "
  ...
  technique_statement <- character(0)
  
  if(str_detect(object, "pedicle_screw|pars_screw|lateral_mass_screw")){
    technique_statement <- c(technique_statement, 
                             "SCREW_TECHNIQUE_PLACEHOLDER")
    }
  
  # {glue_collapse(x = levels_side_df$level_side, sep = ', ', last = ' and ')}
  

  
  technique_statement_all <- case_when(
    ...
    str_detect(object, "si_fusion") ~ glue("{si_fusion_screw_statement}"),
    ...
    object == 'nerve_transection' ~ glue("I then proceeded with the nerve root transection {glue_collapse(x = levels_side_df$level_side, sep = ', ', last = ' and ')}. The nerve root was clearly identified and silk sutures were passed medial and lateral to the DRG. The sutures were then tied and the nerve root transected. This completed the spinal nerve transection {glue_collapse(x = levels_side_df$level_side, sep = ', ', last = ' and ')}.")
    )
  
  technique_statement <- c(technique_statement, 
                           technique_statement_all)
...
op_note_distinct_technique_statement <- function(object, level, side, interbody_statement = NULL){
  ...
  if(length(side$side) > 1){
    side_level_text <- glue("at {level}")
    completion_procedure_text <- 'This was carried out from the left and right side. '
  }else{
    side_level_text <- glue("from the {side$side} at {level}")
    completion_procedure_text <- ''
  }
  
  # side_level_text <- 

  
  # {if_else(length(side$side) > 1, 'This was carried out from the left and ride side. process was completed bilaterally to allow ventral decompression. ')}
  
  technique_statement <- case_when(
    object == 'transpedicular_approach' ~ glue("I then proceeded with a transpedicular approach for decompression {side_level_text}. ",
                                               "Using a combination of a high-speed burr and Kerrison rongeur, the posterior elements were resected and the {level} pedicle was skeletonized and resected. ",
                                               "A cavity was created in the vertebral body, allowing ventral decompression of the {if_else(str_starts(level, 'L'), 'cauda equina and nerve roots', 'spinal cord')} at the {level} level. {completion_procedure_text}"
                                               ),
    object == 'costovertebral_approach' ~ glue("I then proceeded with a costovertebral approach for decompression {side_level_text}. ",
                                               "The {level} rib was identified, skeletonized, and 5-6cm of the rib was resected to allow access to the dorsal vertebral body. ",
                                               "Using a combination of a high-speed burr and Kerrison rongeur, the posterior elements were resected and the {level} pedicle was skeletonized and resected. ",
                                               "A cavity was created in the vertebral body, allowing ventral decompression of the spinal cord at the {level} level. ",
                                               "Following the decompression, the canal was palpated to confirm an adequate decompression had been completed. {completion_procedure_text}"
                                               ),
    object == 'lateral_extraforaminal_approach' ~ glue("I then proceeded with a lateral extraforaminal approach {side_level_text}. ",
                                                       "The pedicle of {level} was identified. The {level} inferior and {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_level} superior facets were identified and the {level} pars was used as reference points. ",
                                                       "Using a combination of a high-speed burr and rongeur, the lateral {level} pars was partially resected, and the {level} nerve root was identified and protected. ",
                                                       "This allowed access to the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_interspace} interspace. ",
                                                       "The lateral border of the disk space was identified and the dissection was carried out laterally and medially until adequate exposure was obtained. ",
                                                       "The retropulsed disc fragment was identified causing the neural compression. ",
                                                       "I then proceed with excision of the disc until an adeqate decompression was completed. {completion_procedure_text}"
                                                       ),
    object == 'lateral_extracavitary_approach' ~ glue("I then proceeded with a modified lateral extracavitary approach {side_level_text}. ",
                                                      "The pedicle of {level} was identified. Using a combination of a high-speed burr and Kerrison rongeur and osteotome, ",
                                                      "the {level} inferior and {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_level} superior facets were fully resected, ",
                                                      "allowing access to the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_interspace} interspace. ",
                                                      "The dissection was carried out to the lateral border of the disk space. The disk space was then entered and the disc excised and endplates prepared for arthrodesis. ",
                                                      "This allowed safe access to the most anterior portion of the vertebral body and disk space. "),
    object == 'revision_transpedicular_approach' ~ glue("I then proceeded with a transpedicular approach for a reexploration and revision decompression {side_level_text}. ",
                                                        "Using a combination of a high-speed burr, curettes, and Kerrison rongeur, the scar and posterior elements were carefully resected until I found the plane between scar and dura. ",
                                                        "The {level} pedicle was skeletonized and resected, to allow ventral decompression of the {if_else(str_starts(level, 'L'), 'cauda equina and nerve roots', 'spinal cord')} at the {level} level. {completion_procedure_text}",
                                                        "Following the revision decompression, the canal was palpated to confirm an adequate decompression had been completed."),
    object == 'revision_costovertebral_approach' ~ glue("I then proceeded with a costovertebral approach for a reexploration and revision decompression {side_level_text}. ",
                                                        "The {level} rib was identified, skeletonized, and 5-6cm of the rib was resected to allow access to the dorsal vertebral body. ",
                                                        "Using a combination of a high-speed burr, curettes, and Kerrison rongeur, the scar and posterior elements were carefully resected until I found the plane between scar and dura. ",
                                                        "The {level} pedicle was skeletonized and resected, to allow full decompression of the spinal cord at the {level} level. {completion_procedure_text}",
                                                        "Following the revision decompression, the canal was palpated to confirm an adequate decompression had been completed."),
    object == 'corpectomy_extracavitary_tumor' ~ glue("I then proceeded with a partial vertebral corpectomy using a modified lateral extracavitary approach {side_level_text}. ",
                                                      "The pedicle of {level} was identified. Using a combination of a high-speed burr and Kerrison rongeur and osteotome, ",
                                                      "the {level} inferior and {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_level} superior facets were resected and the {level} superior and {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} inferior facets were resected as needed to skeletonize the {level} pedicle, ",
                                                      "allowing access to the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_interspace} interspace and {level} vertebral body. ",
                                                      "The lateral border of the disk space was identified and the dissection was carried out laterally and slightly anteriorly over the vertebral body. ",
                                                      "Once the posterior elements were resected and I had gained adequate exposure to the vertebral body via the extracavitary approach and the {level} nerve root could be visualized and protected, ",
                                                      "I used a combination of a burr, rongeur, osteotomes and curettes to resect any visible tumor and vertebral bone, creating a cavity in the vertebral body. {completion_procedure_text}",
                                                      "Once an adequate cavity had been created, the remaining shell of the posterior vertebral body along with the PLL was impacted into the cavity, ",
                                                      "completing the decompression of the {if_else(str_detect(level, 'L'),'cauda equina and nerve roots', 'spinal cord')} and separation of visible tumor by at least 10mm. "),
    object == 'grade_3' ~ glue("I then proceeded with the pedicle subtraction osteotomy at the level of {level}. ",
                               "A central laminectomy was performed and confirmed at the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} level in order to prevent compression after osteotomy closure. ",
                               "First, the necessary posterior elements were resected. ",
                               "The inferior facets of {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} were resected up to the pedicle and then the {level} superior facets were completely resected, allowing the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} nerve roots to be visualized and tracked laterally. ",
                               "Then a cut was made through the pars just inferior to the {level} pedicle and the {level} inferior facet was completely resected along with the superior facet of {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_level}. ",
                               "The {level} nerve root was visualized and carefully tracked laterally. ",
                               "Once the {level} pedicle was completely skeletonized, and the cranial and caudal nerve roots identified and a full decompression was confirmed from the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} pedicle to the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_level} pedicle, a subperiosteal plane was developed around the anterior portion of the {level} body and the retroperitoneal  structures were protected. ",
                               "The pedicle was resected down to the dorsal vertebral body bilaterally. The lateral portion of the pedicle wall was taken down, and adequate vertebral body was resected to allow for closure of the osteotomy, taking care to leave adequate anterior body and cortex. Once an adequate defect had been created in the vertebral body, the dorsal wall of the vertebral body was impacted. ",
                               "The nerve roots were checked again and the underside of the thecal sac was palpated to confirm no bony impingement. ",
                               "Once I was satisfied that the thecal sac was fully decompressed circumferentially and that the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} and {level} nerve roots were completely free, rods were connected bilaterally and the osteotomy was closed in a controlled fashion. ",
                               "The set screws were final tightened to lock the osteotomy in position. Intraoperative X-ray was performed to confirm appropriate correction through the osteotomy site."),    
    object == 'grade_4' ~ glue("I then proceeded with an extended three column osteotomy (partial corpectomy) at the level of {level}. The posterior elements of {level} were completely removed. A complete central laminectomy was confirmed at the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} level in order to prevent compression after osteotomy closure. ",
                               "Once I was satisfied that an adequate decompression had been performed proximally and distally, I worked to skeletonize the {level} pedicles. The {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} inferior facet and pars were resected bilaterally up to the level of the pedicle, exposing the exiting {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} nerve roots. ",
                               "I tracked these laterally to be sure there was no compression or dorsal bone that could cause compression after osteotomy closure. The {level} pars and inferior facet, and {jh_get_cranial_caudal_interspace_body_list_function(level = level)$caudal_level} superior facet were resected, exposing the {level} exiting nerve roots bilaterally. ",
                               "Again, I tracked the nerve roots laterally to confirm no sites of compression. ",
                               "At this point, the pedicles were skeletonized bilaterally. I developed a plane along laterally around the anterior portion of the {level} body to stay dorsal to the retroperitoneal structures. ",
                               "Then, each pedicle was fully resected along with the dorsal and superior vertebral body. I extended the resection into the disc space, partially resecting the superior {level} endplate along with the disc. Once roughly 50% of the vertebral body had been resected, the nerve roots were checked again and the underside of the thecal sac was palpated to confirm no bony impingement, and the dorsal wall of the vertebral body was impacted.  ",
                               "Once I was satisfied that the thecal sac was fully decompressed circumferentially and that the {jh_get_cranial_caudal_interspace_body_list_function(level = level)$cranial_level} and {level} nerve roots were completely free, rods were connected bilaterally and the osteotomy was closed in a controlled fashion. ",
                               "Intraoperative X-ray was performed to confirm appropriate alignment and once I was satisfied with the alignment, the set screws across the osteotomy site were final tightened. "),
    object == 'grade_5' ~ glue("I then proceeded with a vertebral column resection at the level of {level}. ",
                               "The {level} pedicle was skeletonized and the posterior elements from the {cranial_level} pedicle to the {caudal_level} pedicle were resected. ",
                               "Adequate {level} rib was exposed and resected. The {level} pedicle was resected allowing access to the dorsal vertebral body. {thoracic_nerve_ligation_statement}", 
                               " Prior to any destabilization, a unilateral stabilizing rod was placed on the contralateral side. ",
                               "A combination of blunt and electrocautery dissection was used to develop a subperiosteal plane around the entire anterior portion of the {level} body and the anterior and neural structures were protected. ",
                               "A window was made in the vertebral body and the cancellous vertebral body was removed from cranial to caudal disk space. ",
                               "The process was repeated on the opposite side until the entire vertebral body was resected up to the ALL and the dorsal vertebral wall was fully resected to ensure to impingement on the thecal sac while closing the osteotomy. ",
                               "Adequate laminectomy was confirmed at the cranial and caudal levels to ensure no compression while closing the osteotomy. ",
                               "We then turned toward closure of the resection site. A combination of compression, in-situ rod bending and temporary rods were used to close the osteotomy site in a controlled manner, taking care to regularly check tcMEPs. ",
                               "Once I was satisfied with the closure, rods were secured into place bilaterally with set screws. "
    ),
    object == 'no_implant_interbody_fusion' ~ glue("I then proceeded with the interbody fusion {side_level_text}. ",
                                                   "The inferior and superior facets of {str_replace_all(level, '-', ' and ')}, respectively, were resected and the exiting and traversing nerve roots were appropriately protected. ",
                                                   "The annulus was incised, allowing access to the disk space. ",
                                                   "Using a combination of curettes, shavers, and pituitary rongeur, the disk was excised and the endplates were prepped for an interbody fusion."),
    object == 'plif' ~ glue("I then proceeded with the posterior lumbar interbody fusion (PLIF) procedure and placement of interbody implant at the level of {level}. ",
                            "The inferior and superior facets of {str_replace_all(level, '-', ' and ')}, respectively, were partially resected and the exiting and traversing nerve roots were appropriately protected. ",
                            "The dura was retracted in order to access the disc space. The annulus was incised, allowing access to the disk space. ",
                            "The disk was excised and endplates were prepped using a combination of curettes, shavers, and pituitary rongeur. ",
                            "Once the endplates were adequately prepped, trials were used to determine the appropriate size of the implant. ",
                            "Once I was satisfied with the size of the trial, the interbody implant was placed into the interbody space along with the bone graft. {interbody_statement} ",
                            "The final position of the implant was confirmed using intraoperative X-ray."),
    object == "tlif" ~ glue("I then proceeded with the transforaminal interbody fusion (TLIF) procedure and placement of interbody implant {side_level_text}. ",
                            "The inferior and superior facets of {str_replace_all(level, '-', ' and ')}, respectively, were resected and the exiting and traversing nerve roots were appropriately protected. ",
                            "The annulus was incised, allowing access to the disk space. The disk was excised and endplates were prepped using a combination of curettes, shavers, and pituitary rongeur. ",
                            "Once the endplates were adequately prepped, trials/shavers were used to determine the appropriate size of the implant. ",
                            "Once I was satisfied with the size of the trial, the interbody implant was placed into the interbody space along with the bone graft. {interbody_statement} ",
                            "The final position of the implant was confirmed using intraoperative X-ray."),
    object == "intervertebral_cage" ~ glue("I then proceed with placement of the intervertebral cage. ",
                                           "After ensuring the neural structures were completely free and the bony surfaces cranially and caudally were appropriately prepared, the intervertebral distance was measured. {interbody_statement} ",
                                           "The implant was then inserted into the defect, abutting bone superiorly and inferiorly. Intraoperative X-ray was used to confirm appropriate size, fit, and alignment of the intervertebral implant spanning {level}."),
    object == "removal_scs_paddle" ~ glue("I then proceeded with removing the spinal cord stimulator paddle. The dissection was carried out until I encountered the leads. The leads were released from the soft tissue and the interspace was identified and. ",
                                          "The leads were tracked down to the paddle device, which was then removed."),
    object == "removal_scs_perc_array" ~ glue("I then proceeded with removing the spinal cord stimulator percutaneous array. The dissection was carried out until I encountered the leads. The leads were released from the soft tissue and the interspace was identified. ",
                                              "The array was then removed without difficulty."),
    object == "removal_scs_receiver" ~ glue("I then proceeded with removing the spinal cord stimulator pulse generator/receiver. The prior incision was used and dissection was carried down to the receiver. ",
                                            "The soft tissue was released around the receiver and sutures excised until it was completely free and the device was removed. The dead space was then adequately closed.")
  )
...
op_note_procedures_performed_numbered_function <- function(objects_added_df,
  ...
  if(any(unique(fusion_levels_vector) == "Sacro-iliac") & any(str_detect(objects_added_df$object, "si_fusion"))){
    fusion_levels_vector <- fusion_levels_vector[fusion_levels_vector != "Sacro-iliac"]
  }
  ...
  summary_multiple_nested <- summary_nested_df %>%
    ...
    mutate(procedure_performed_statement = case_when(
      procedure_class == "Pelvic instrumentation" ~ paste("Instrumentation of the Pelvis with", levels, "fixation"),
      procedure_class == "SI Fusion" ~ "SI Fusion with transfixing device (27279)",
      TRUE ~ as.character(procedure_performed_statement)
    )
    ) %>%
    # mutate(procedure_performed_statement = if_else(procedure_class == "Pelvic instrumentation", paste("Instrumentation of the Pelvis with", levels, "fixation"), as.character(procedure_performed_statement))) %>%
    select(procedure_class, procedure_performed_statement)%>%
